class EventsController < ApplicationController
  ...
  def job_event_to_notification(event)
    ...
    if event.notify_before
      ...
      NotifyEventUserBeforeJob.set(wait_until: event.start_time - event.minutes).perform_later(event.id)
      # call the Job the the caretakers of the user of the event (event.user.caretakers)
      NotifyCaretakerEventUserBeforeJob.set(wait_until: event.start_time - event.minutes).perform_later(event.id)
    end
